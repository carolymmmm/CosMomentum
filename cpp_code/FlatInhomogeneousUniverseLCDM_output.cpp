...
vector<vector<double> > FlatInhomogeneousUniverseLCDM::return_LOS_integrated_phi_of_lambda(double theta, double f_NL, double var_NL_rescale, vector<double> w_values, vector<double> kernel_values){
  ...
  //vector<double>::iterator kernel_max_iterator = std::max_element(kernel_values[t].begin(), kernel_values[t].end());
  ...
  for(int t = 0; t < n_time_of_nonzero_kernel; t++){
    ...
    for(int l = 0; l < n_lambda; l++){
      y_values[t][l] /= abs(kernel_values[i]);
    }
    ...
    //cout << w << "   " << *y_max_iterator << "   " << y_max << "   " << kernel_values[i] << "  <---------- \n";
    ...
    if(kernel_values[i] < 0.0 && 1==1){
      //cout << "HAHAHAHA 1: " << *y_max_iterator << '\n';
      
      std::reverse(y_values[t].begin(), y_values[t].end());
      std::reverse(phi_values[t].begin(), phi_values[t].end());
      std::reverse(phi_prime_values[t].begin(), phi_prime_values[t].end());
      std::reverse(phi_prime_prime_values[t].begin(), phi_prime_prime_values[t].end());
      std::reverse(phi_prime_prime_prime_values[t].begin(), phi_prime_prime_prime_values[t].end());
      std::reverse(bias_term_1_values[t].begin(), bias_term_1_values[t].end());
      std::reverse(bias_term_2_values[t].begin(), bias_term_2_values[t].end());
      std::reverse(variance_term_1_values[t].begin(), variance_term_1_values[t].end());
      std::reverse(variance_term_2_values[t].begin(), variance_term_2_values[t].end());
      std::reverse(variance_term_3_values[t].begin(), variance_term_3_values[t].end());
      //
      for(int l = 0; l < y_values[t].size(); l++){
        y_values[t][l] *= -1;
      }
      //cout << "HAHAHAHA 2: " << *y_max_iterator << '\n';
    }
...
/*******************************************************************************************************************************************************
 * return_LOS_integrated_phi_of_lambda_incl_galaxies
 * Description:
 * - set phi(delta, eta) and lambda(delta, eta) on a 2D grid. This grid is used when computing the LOS-projected CGF in Limber approximation
 * Arguments:
 * - double theta: angular top-hat radius with which LOS-integrated field is smoothed
...
vector<vector<double> > FlatInhomogeneousUniverseLCDM::return_LOS_integrated_phi_of_lambda_incl_galaxies(double theta, double f_NL, double var_NL_rescale, vector<double> w_values, vector<double> kernel_values, vector<double> kernel_values_lenses){
  ...
  //cout << "computing CGF grid & cutting out 1st branch\n";
  //cout << "n_time_of_nonzero_kernel: " << n_time_of_nonzero_kernel << '\n';
  ...
  for(int t = 0; t < n_time_of_nonzero_kernel; t++){
    ...
    //cout << w << "   " << *y_max_iterator << "   " << y_max << "   " << i << "   " << kernel_values[i] << "<---------- \n";
   ...
   * 
   */
  vector<vector<double> > projected_phi_data(8, vector<double>(n_lambda_1st_branch, 0.0));
  projected_phi_data[0] = vector<double>(&y_values[time_index_y_max][lambda_index_y_min], &y_values[time_index_y_max][lambda_index_y_max]+1);
  ...
  for(int y = 0; y < n_lambda_1st_branch; y++){
    for(int t = 0; t < n_time_of_nonzero_kernel; t++){
      ...
      kernel_times_dw = dw*kernel_values[i];
      kernelSq_times_dw = kernel_times_dw*kernel_values[i];
      kernel_lenses_times_dw = dw*kernel_values_lenses[i];
      projected_phi_data[1][y] += interpolate_neville_aitken(projected_phi_data[0][y], &y_values[t], &phi_values[t], constants::order_of_interpolation)*dw;
      projected_phi_data[2][y] += interpolate_neville_aitken(projected_phi_data[0][y], &y_values[t], &phi_prime_values[t], constants::order_of_interpolation)*kernel_times_dw;
      projected_phi_data[3][y] += interpolate_neville_aitken(projected_phi_data[0][y], &y_values[t], &phi_prime_prime_values[t], constants::order_of_interpolation)*kernelSq_times_dw;
      projected_phi_data[4][y] += interpolate_neville_aitken(projected_phi_data[0][y], &y_values[t], &phi_prime_prime_prime_values[t], constants::order_of_interpolation)*kernel_values[i]*kernelSq_times_dw;
      // Now bias terms:
      // <delta_g | kappa > = invLaplace[<delta_m | kappa > + b_1_Lagrange*bias_term_1 + b_2_Lagrange*bias_term_2]
      projected_phi_data[5][y] += interpolate_neville_aitken(projected_phi_data[0][y], &y_values[t], &bias_term_1_values[t], constants::order_of_interpolation)*kernel_lenses_times_dw;
      projected_phi_data[6][y] += interpolate_neville_aitken(projected_phi_data[0][y], &y_values[t], &bias_term_2_values[t], constants::order_of_interpolation)*kernel_lenses_times_dw;
      // this gives <delta_m | kappa >
      projected_phi_data[7][y] += interpolate_neville_aitken(projected_phi_data[0][y], &y_values[t], &phi_prime_values[t], constants::order_of_interpolation)*kernel_lenses_times_dw;
    }
  }
  
  //cout << "lambda_min = " << projected_phi_data[0][0] << '\n';
  //cout << "lambda_max = " << projected_phi_data[0][n_lambda_1st_branch-1] << '\n';
  //cout << "phi_min = " << projected_phi_data[1][0] << '\n';
  //cout << "phi_max = " << projected_phi_data[1][n_lambda_1st_branch-1] << '\n';
  //cout << "delta_min = " << projected_phi_data[2][0] << '\n';
  //cout << "delta_max = " << projected_phi_data[2][n_lambda_1st_branch-1] << '\n';
...
vector<vector<double> > FlatInhomogeneousUniverseLCDM::compute_LOS_projected_PDF(vector<double> w_values, vector<double> kernel_values, double theta, double f_NL, double var_NL_rescale){
  ...
  //cout << "<------- FRANCIS start ------->\n";
  //return this->compute_PDF_from_CGF_Francis(&delta_values, &CGF_data);
...
/*
 * FlatInhomogeneousUniverseLCDM::compute_LOS_projected_PDF_incl_galaxies
 * 
 * Returns PDF of line-of-sight projected matter density contrast with projection kernel specified by the input arrays w_values and kernel_values. w_values should store the lower-redshift edge of each histogram bin and n_of_w_values should contain the redshift histrogram (the histogram doesn't need to be normalised, since normalisation is enforced later on in the code). First column of the returned array is \delta smoothed with a spherical top-hat of R_in_Mpc_over_h, while 2nd column is p(\delta).
 * 
 */
...
vector<vector<double> > FlatInhomogeneousUniverseLCDM::compute_LOS_projected_PDF_incl_galaxies(vector<double> w_values, vector<double> kernel_values, vector<double> kernel_values_lenses, double theta, double f_NL, double var_NL_rescale){
  
  //cout << "Computing projected phi_data:\n";
  //cout.flush();
    
  vector<vector<double> > CGF_data = this->return_LOS_integrated_phi_of_lambda_incl_galaxies(theta, f_NL, var_NL_rescale, w_values, kernel_values, kernel_values_lenses);
  ...
  cout << "starting compute_PDF_from_CGF_no_variance_terms\n";
  return this->compute_PDF_from_CGF_no_variance_terms(&delta_values, &CGF_data);
...
vector<vector<double> > FlatInhomogeneousUniverseLCDM::compute_LOS_projected_PDF_saddle_point(vector<double> w_values, vector<double> kernel_values, double theta, double f_NL, double var_NL_rescale, int next_to_leading){
  ...
  for(int i = 0; i < n_delta; i++){
    ...
    if(next_to_leading_correction < 1.3 && next_to_leading_correction > 0.7 && next_to_leading == 1)
...
void FlatInhomogeneousUniverseLCDM::return_LOS_integrated_phi_of_lambda_incl_CMB_kappa(double theta, double f_NL, double var_NL_rescale, vector<double> w_values, vector<double> kernel_values, vector<vector<double> > *phi_data_delta, vector<vector<double> > *phi_data_kappa, vector<vector<double> > *phi_grid, vector<vector<double> > *dphi_dldelta_grid, vector<vector<double> > *dphi_dlkappa_grid, vector<vector<double> > *d2phi_dldelta2_grid, vector<vector<double> > *d3phi_dldelta3_grid, vector<vector<double> > *d3phi_dldelta2_dlkappa_grid, vector<vector<double> > *d3phi_dldelta_dlkappa2_grid, vector<vector<double> > *d4phi_dldelta4_grid, vector<vector<double> > *d4phi_dldelta2_dlkappa2_grid, vector<vector<double> > *d2phi_dldelta_dlkappa_grid, vector<vector<double> > *d2phi_dlkappa2_grid, vector<vector<int> > *grid_mask){
  ...
  double z_min, z_max;
  double norm = 0.0;
  vector<vector<double> > Takahashi_kernel(0, vector<double>(0, 0.0));
  if(this->Takahashi_shell_thickness == 1){
    Takahashi_kernel = read_table_double_transposed("./Data/joint_PDFs_for_hack/Takahashi_kernel.dat");
  }
  ...
  for(int i = 0; i < n_time; i++){
    ...
    cout << w << "   " << CMB_lensing_kernel[i] << "   ";
    if(this->Takahashi_shell_thickness == 1){
      CMB_lensing_kernel[i] = interpolate_neville_aitken(w, &Takahashi_kernel[0], &Takahashi_kernel[1], 1);//constants::order_of_interpolation);
      if(CMB_lensing_kernel[i] < 0.0) CMB_lensing_kernel[i] = 0.0;
    }
    cout << CMB_lensing_kernel[i] << " <----- Now linear\n";
  }
  ...
  for(int d = 0; d < N_lambda_delta; d++){
    for(int k = 0; k < N_lambda_kappa; k++){
      ...
      if(determinant > 0.0 && d2phi_dldelta2 > 0.0 && d2phi_dlkappa2 > 0.0 && grid_mask[d][k] == 1){
        ...
        next_to_leading_correction = 1.0;
...
/*******************************************************************************************************************************************************
 * return_LOS_joint_PDF_deltaG_kappa
 * 
 * 
*******************************************************************************************************************************************************/

vector<vector<double> > FlatInhomogeneousUniverseLCDM::return_LOS_joint_PDF_deltaG_kappa(double theta, double f_NL, double var_NL_rescale, double var_kappa_noise, vector<double> w_values, vector<double> kernel_values, vector<double> lensing_kernel_values){
  ...
  vector<vector<double> > coefficients_phi_prime_of_tau(n_time, vector<double>(n_coeff, 0.0));
  vector<vector<double> > coefficients_dphi_prime_dtau(n_time, vector<double>(n_coeff, 0.0));
  vector<vector<double> > coefficients_d2phi_prime_dtau2(n_time, vector<double>(n_coeff, 0.0));
  ...
  vector<vector<double> > phi_data_delta = this->return_LOS_integrated_phi_of_lambda(theta, f_NL, var_NL_rescale, w_values, kernel_values);
  vector<vector<double> > phi_data_kappa = this->return_LOS_integrated_phi_of_lambda(theta, f_NL, var_NL_rescale, w_values, lensing_kernel_values);
  
  vector<double> lambda_delta_values = phi_data_delta[0];
  vector<double> d2phi_delta_dlambda2 = phi_data_delta[3];
  //
  vector<double> lambda_kappa_values = phi_data_kappa[0];
  vector<double> d2phi_kappa_dlambda2 = phi_data_kappa[3];
  
  
  /*
   * getting delta_values
   * 
   */
  int n_delta = constants::N_delta_values_for_PDFs;
  double var_delta = interpolate_neville_aitken(0.0, &lambda_delta_values, &d2phi_delta_dlambda2, constants::order_of_interpolation);
  double skew_delta = interpolate_neville_aitken_derivative(0.0, &lambda_delta_values, &d2phi_delta_dlambda2, constants::order_of_interpolation);
  double lognormal_shift_delta = lognormal_tools::get_delta0(var_delta, skew_delta);
  double var_Gauss_delta = log(1.0+var_delta/lognormal_shift_delta/lognormal_shift_delta);
  double mean_Gauss_delta = -0.5*var_Gauss_delta;
  double std_Gauss_delta = sqrt(var_Gauss_delta);
  double d_min = max(-1.0, lognormal_shift_delta*(exp(mean_Gauss_delta-5.0*std_Gauss_delta)-1.0));
  double d_max = lognormal_shift_delta*(exp(mean_Gauss_delta+5.0*std_Gauss_delta)-1.0);
  double ddelta = (d_max-d_min)/double(n_delta);
  /*
   * getting kappa_values
   * 
   */
  int n_kappa = constants::N_delta_values_for_PDFs;
  double var_kappa = interpolate_neville_aitken(0.0, &lambda_kappa_values, &d2phi_kappa_dlambda2, constants::order_of_interpolation);
  double skew_kappa = interpolate_neville_aitken_derivative(0.0, &lambda_kappa_values, &d2phi_kappa_dlambda2, constants::order_of_interpolation);
  double lognormal_shift_kappa = lognormal_tools::get_delta0(var_kappa, skew_kappa);
  double var_Gauss_kappa = log(1.0+var_kappa/lognormal_shift_kappa/lognormal_shift_kappa);
  double mean_Gauss_kappa = -0.5*var_Gauss_kappa;
  double std_Gauss_kappa = sqrt(var_Gauss_kappa);
  double k_min = max(-1.0, lognormal_shift_kappa*(exp(mean_Gauss_kappa-5.0*std_Gauss_kappa)-1.0));
  double k_max = lognormal_shift_kappa*(exp(mean_Gauss_kappa+5.0*std_Gauss_kappa)-1.0);
  k_min = -sqrt(k_min*k_min + 4.0*var_kappa_noise);
  k_max = sqrt(k_max*k_max + 4.0*var_kappa_noise);
  var_kappa += var_kappa_noise;
  double dkappa = (k_max-k_min)/double(n_kappa);
  
  vector<double> delta_values(n_delta, 0.0);
  vector<double> kappa_values(n_kappa, 0.0);
  for(int i = 0; i < n_delta; i++){
    delta_values[i] = d_min + double(i)*ddelta;
    kappa_values[i] = k_min + double(i)*dkappa;
  }
  ...
  for(int t = 0; t < n_time; t++){
    ...
    this->compute_tau_coefficients(&y_values[t], &phi_values[t], &phi_prime_values[t], &phi_prime_prime_values[t], &coefficients_phi_prime_of_tau[t], &coefficients_dphi_prime_dtau[t], &coefficients_d2phi_prime_dtau2[t]);
  ...
  complex<double> lambda, lambda_delta, lambda_kappa;
  complex<double> tau;
  complex<double> phi_prime;
  complex<double> integrand, exponent;
  
  vector<vector<double> > PDF_data(n_delta+1, vector<double>(n_kappa+1));
  ...
  double delta_max = delta_values[n_delta-1];
  delta_max = max(-delta_min, delta_max);
  delta_min = -delta_max;
  double kappa_min = kappa_values[0];
  double kappa_max = kappa_values[n_kappa-1];
  kappa_max = max(-kappa_min, kappa_max);
  kappa_min = -kappa_max;
  
  //double sigma_frac = 0.01;
  double sigma_frac = 1.0;
  double dr_delta = sigma_frac*min(0.1/sqrt(var_delta), 0.5/delta_max);
  double dr_kappa = sigma_frac*min(0.1/sqrt(var_kappa), 0.5/kappa_max);
  int log2n = 9;
  int n = pow(2, log2n);
  int n_half = pow(2, log2n-1);
  // from toolkits paper:
  dr_delta = sqrt(2.0*constants::pi/(n*var_delta));
  dr_kappa = sqrt(2.0*constants::pi/(n*var_kappa));
  double lambda_delta_max = n_half*dr_delta;
  double lambda_delta_min = -lambda_delta_max;
  double dl_delta = (lambda_delta_max-lambda_delta_min)/n;
  double lambda_kappa_max = n_half*dr_kappa;
  double lambda_kappa_min = -lambda_kappa_max;
  double dl_kappa = (lambda_kappa_max-lambda_kappa_min)/n;
    
  double lambda_d_values[n];
  double lambda_k_values[n];
  double integrand_re[n*n];
  double integrand_im[n*n];
  double d_values[n];
  double k_values[n];
  double p_re[n*n];
  double p_im[n*n];
  ...
  for(int i = 0; i < n; i++){
    lambda_d_values[i] = lambda_delta_min + i*dl_delta;
    lambda_k_values[i] = lambda_kappa_min + i*dl_kappa;
  }
  ...
  for(int id = 0; id < n; id++){
    //cout << "\r" << int(10000*id/double(n))/100.0 << "%";
    //cout.flush();
    lambda_delta = complex<double>(0.0, lambda_d_values[id]);
    for(int ik = 0; ik < n; ik++){
      lambda_kappa = complex<double>(0.0, lambda_k_values[ik]);
      exponent = 0.0;
      tau = 0.0;
      for(int t = 0; t < n_time; t++){
        dw = w_values[t+1]-w_values[t];
        kernel_delta = kernel_values[t];
        kernel_kappa = lensing_kernel_values[t];
        lambda = kernel_delta*lambda_delta + kernel_kappa*lambda_kappa;
        //
        tau = get_tau_from_secant_method_complex_Bernardeau_notation_2D(lambda, tau, &coefficients_dphi_prime_dtau[t], &coefficients_d2phi_prime_dtau2[t]);
        phi_prime = return_polnomial_value(tau, &coefficients_phi_prime_of_tau[t]);
        exponent += dw*(lambda*phi_prime-0.5*pow(tau, 2));
      }
      exponent += 0.5*var_kappa_noise*lambda_kappa*lambda_kappa;
      //cout << id << "   ";
      //cout << ik << "   ";
      //cout << lambda_delta << "   ";
      //cout << lambda_kappa << "   ";
      ...
      //cout << integrand << "\n";
      integrand_re[n*id+ik] = integrand.real();
      integrand_im[n*id+ik] = integrand.imag();
    }
  }
  cout << "\r100%\n                   ";
  ...
  fourier_transform_2D(lambda_d_values, lambda_k_values, integrand_re, integrand_im, d_values, k_values, p_re, p_im, log2n);
  
  
  
  vector<double> delta_for_interpolation(n, 0.0);
  vector<double> kappa_for_interpolation(n, 0.0);
  vector<vector<double> > PDF_for_interpolation(n, vector<double>(n, 0.0));
  
  for(int i = 0; i < n; i++){
    delta_for_interpolation[i] = d_values[i];
    kappa_for_interpolation[i] = k_values[i];
    for(int j = 0; j < n; j++){
      PDF_for_interpolation[i][j] = p_re[i*n+j];
      //PDF_for_interpolation[i][j] = integrand_re[i*n+j];
    }
  }
  ...
  for(int i = 0; i < n_delta; i++){
    PDF_data[i+1][0] = delta_values[i];
    //PDF_data[i+1][0] = delta_for_interpolation[i];
    for(int j = 0; j < n_kappa; j++){
      PDF_data[0][j+1] = kappa_values[j];
      //PDF_data[0][j+1] = kappa_for_interpolation[j];
      //PDF_data[i+1][j+1] = PDF_for_interpolation[i][j]; 
      //PDF_data[i+1][j+1] = interpolate_neville_aitken_grid(delta_values[i], kappa_values[j], &delta_for_interpolation, &kappa_for_interpolation, &PDF_for_interpolation, constants::order_of_interpolation, constants::order_of_interpolation);
      //PDF_data[i+1][j+1] = interpolate_grid_biquadratic(delta_values[i], kappa_values[j], &delta_for_interpolation, &kappa_for_interpolation, &PDF_for_interpolation);
      PDF_data[i+1][j+1] = interpolate_grid_biquadratic(delta_values[i], kappa_values[j], &delta_for_interpolation, &kappa_for_interpolation, &PDF_for_interpolation);
    }
  }
  
  cout << "var_delta: " << var_delta << '\n';
  cout << "var_kappa: " << var_kappa << '\n';
  cout << "skew_delta: " << skew_delta << '\n';
  cout << "skew_kappa: " << skew_kappa << '\n';
...
/*******************************************************************************************************************************************************
 * return_LOS_joint_PDF_kappa_kappa
 * 
 * 
*******************************************************************************************************************************************************/

vector<vector<double> > FlatInhomogeneousUniverseLCDM::return_LOS_joint_PDF_kappa_kappa(double theta, double f_NL, double var_NL_rescale, double var_kappa_noise_1, double var_kappa_noise_2, vector<double> w_values, vector<double> lensing_kernel_values_1, vector<double> lensing_kernel_values_2){
  ...
  vector<vector<double> > phi_data_kappa_1 = this->return_LOS_integrated_phi_of_lambda(theta, f_NL, var_NL_rescale, w_values, lensing_kernel_values_1);
  vector<vector<double> > phi_data_kappa_2 = this->return_LOS_integrated_phi_of_lambda(theta, f_NL, var_NL_rescale, w_values, lensing_kernel_values_2);
  
  vector<double> lambda_kappa_1_values = phi_data_kappa_1[0];
  vector<double> d2phi_kappa_1_dlambda2 = phi_data_kappa_1[3];
  //
  vector<double> lambda_kappa_2_values = phi_data_kappa_2[0];
  vector<double> d2phi_kappa_2_dlambda2 = phi_data_kappa_2[3];
   ...
   * 
   */
  int n_kappa_1 = constants::N_delta_values_for_PDFs;
  double var_kappa_1 = interpolate_neville_aitken(0.0, &lambda_kappa_1_values, &d2phi_kappa_1_dlambda2, constants::order_of_interpolation);
  double skew_kappa_1 = interpolate_neville_aitken_derivative(0.0, &lambda_kappa_1_values, &d2phi_kappa_1_dlambda2, constants::order_of_interpolation);
  double lognormal_shift_kappa_1 = lognormal_tools::get_delta0(var_kappa_1, skew_kappa_1);
  double var_Gauss_kappa_1 = log(1.0+var_kappa_1/lognormal_shift_kappa_1/lognormal_shift_kappa_1);
  double mean_Gauss_kappa_1 = -0.5*var_Gauss_kappa_1;
  double std_Gauss_kappa_1 = sqrt(var_Gauss_kappa_1);
  double k1_min = max(-1.0, lognormal_shift_kappa_1*(exp(mean_Gauss_kappa_1-5.0*std_Gauss_kappa_1)-1.0));
  double k1_max = lognormal_shift_kappa_1*(exp(mean_Gauss_kappa_1+5.0*std_Gauss_kappa_1)-1.0);
  k1_min = -sqrt(k1_min*k1_min + 4.0*var_kappa_noise_1);
  k1_max = sqrt(k1_max*k1_max + 4.0*var_kappa_noise_1);
  var_kappa_1 += var_kappa_noise_1;
  double dkappa_1 = (k1_max-k1_min)/double(n_kappa_1);
  /*
   * getting kappa_values
   * 
   */
  int n_kappa_2 = constants::N_delta_values_for_PDFs;
  double var_kappa_2 = interpolate_neville_aitken(0.0, &lambda_kappa_2_values, &d2phi_kappa_2_dlambda2, constants::order_of_interpolation);
  double skew_kappa_2 = interpolate_neville_aitken_derivative(0.0, &lambda_kappa_2_values, &d2phi_kappa_2_dlambda2, constants::order_of_interpolation);
  double lognormal_shift_kappa_2 = lognormal_tools::get_delta0(var_kappa_2, skew_kappa_2);
  double var_Gauss_kappa_2 = log(1.0+var_kappa_2/lognormal_shift_kappa_2/lognormal_shift_kappa_2);
  double mean_Gauss_kappa_2 = -0.5*var_Gauss_kappa_2;
  double std_Gauss_kappa_2 = sqrt(var_Gauss_kappa_2);
  double k2_min = max(-1.0, lognormal_shift_kappa_2*(exp(mean_Gauss_kappa_2-5.0*std_Gauss_kappa_2)-1.0));
  double k2_max = lognormal_shift_kappa_2*(exp(mean_Gauss_kappa_2+5.0*std_Gauss_kappa_2)-1.0);
  k2_min = -sqrt(k2_min*k2_min + 4.0*var_kappa_noise_2);
  k2_max = sqrt(k2_max*k2_max + 4.0*var_kappa_noise_2);
  var_kappa_2 += var_kappa_noise_2;
  double dkappa_2 = (k2_max-k2_min)/double(n_kappa_2);
  
  vector<double> kappa_1_values(n_kappa_1, 0.0);
  vector<double> kappa_2_values(n_kappa_2, 0.0);
  for(int i = 0; i < n_kappa_1; i++){
    kappa_1_values[i] = k1_min + double(i)*dkappa_1;
  }
  for(int i = 0; i < n_kappa_2; i++){
    kappa_2_values[i] = k2_min + double(i)*dkappa_2;
  }
  ...
  complex<double> lambda, lambda_kappa_1, lambda_kappa_2;
  complex<double> tau;
  complex<double> phi_prime;
  complex<double> integrand, exponent;
  
  vector<vector<double> > PDF_data(n_kappa_1+1, vector<double>(n_kappa_2+1));
  ...
  double kappa_1_min = kappa_1_values[0];
  double kappa_1_max = kappa_1_values[n_kappa_1-1];
  kappa_1_max = max(-kappa_1_min, kappa_1_max);
  kappa_1_min = -kappa_1_max;
  double kappa_2_min = kappa_2_values[0];
  double kappa_2_max = kappa_2_values[n_kappa_2-1];
  kappa_2_max = max(-kappa_2_min, kappa_2_max);
  kappa_2_min = -kappa_2_max;
  
  //double sigma_frac = 0.01;
  double sigma_frac = 1.0;
  double dr_kappa_1 = sigma_frac*min(0.1/sqrt(var_kappa_1), 0.5/kappa_1_max);
  double dr_kappa_2 = sigma_frac*min(0.1/sqrt(var_kappa_2), 0.5/kappa_2_max);
  int log2n = 9;
  int n = pow(2, log2n);
  int n_half = pow(2, log2n-1);
  // from toolkits paper:
  dr_kappa_1 = sqrt(2.0*constants::pi/(n*var_kappa_1));
  dr_kappa_2 = sqrt(2.0*constants::pi/(n*var_kappa_2));
  double lambda_kappa_1_max = n_half*dr_kappa_1;
  double lambda_kappa_1_min = -lambda_kappa_1_max;
  double dl_kappa_1 = (lambda_kappa_1_max-lambda_kappa_1_min)/n;
  double lambda_kappa_2_max = n_half*dr_kappa_2;
  double lambda_kappa_2_min = -lambda_kappa_2_max;
  double dl_kappa_2 = (lambda_kappa_2_max-lambda_kappa_2_min)/n;
    
  double lambda_k1_values[n];
  double lambda_k2_values[n];
  double integrand_re[n*n];
  double integrand_im[n*n];
  double k1_values[n];
  double k2_values[n];
  ...
  for(int i = 0; i < n; i++){
    lambda_k1_values[i] = lambda_kappa_1_min + i*dl_kappa_1;
    lambda_k2_values[i] = lambda_kappa_2_min + i*dl_kappa_2;
  }
  
  double kernel_kappa_1, kernel_kappa_2;
  
  
  for(int ik1 = 0; ik1 < n; ik1++){
    cout << "\r" << int(10000*ik1/double(n))/100.0 << "%";
    cout.flush();
    lambda_kappa_1 = complex<double>(0.0, lambda_k1_values[ik1]);
    for(int ik2 = 0; ik2 < n; ik2++){
      lambda_kappa_2 = complex<double>(0.0, lambda_k2_values[ik2]);
      exponent = 0.0;
      tau = 0.0;
      for(int t = 0; t < n_time; t++){
        dw = w_values[t+1]-w_values[t];
        kernel_kappa_1 = lensing_kernel_values_1[t];
        kernel_kappa_2 = lensing_kernel_values_2[t];
        lambda = kernel_kappa_1*lambda_kappa_1 + kernel_kappa_2*lambda_kappa_2;
        //
        tau = get_tau_from_secant_method_complex_Bernardeau_notation_2D(lambda, tau, &coefficients_dphi_prime_dtau[t], &coefficients_d2phi_prime_dtau2[t]);
        phi_prime = return_polnomial_value(tau, &coefficients_phi_prime_of_tau[t]);
        exponent += dw*(lambda*phi_prime-0.5*pow(tau, 2));
      }
      exponent += 0.5*var_kappa_noise_1*lambda_kappa_1*lambda_kappa_1;
      exponent += 0.5*var_kappa_noise_2*lambda_kappa_2*lambda_kappa_2;
      //cout << id << "   ";
      //cout << ik << "   ";
      //cout << lambda_delta << "   ";
      //cout << lambda_kappa << "   ";
      ...
      integrand = exp(exponent);
      //cout << integrand << "\n";
      integrand_re[n*ik1+ik2] = integrand.real();
      integrand_im[n*ik1+ik2] = integrand.imag();
    }
  }
  cout << "\r100%\n                   ";
  ...
  fourier_transform_2D(lambda_k1_values, lambda_k2_values, integrand_re, integrand_im, k1_values, k2_values, p_re, p_im, log2n);
  
  
  
  vector<double> kappa_1_for_interpolation(n, 0.0);
  vector<double> kappa_2_for_interpolation(n, 0.0);
  vector<vector<double> > PDF_for_interpolation(n, vector<double>(n, 0.0));
  
  for(int i = 0; i < n; i++){
    kappa_1_for_interpolation[i] = k1_values[i];
    kappa_2_for_interpolation[i] = k2_values[i];
    for(int j = 0; j < n; j++){
      PDF_for_interpolation[i][j] = p_re[i*n+j];
    }
  }
  ...
  for(int i = 0; i < n_kappa_1; i++){
    PDF_data[i+1][0] = kappa_1_values[i];
    //PDF_data[i+1][0] = delta_for_interpolation[i];
    for(int j = 0; j < n_kappa_2; j++){
      PDF_data[0][j+1] = kappa_2_values[j];
      //PDF_data[0][j+1] = kappa_for_interpolation[j];
      //PDF_data[i+1][j+1] = PDF_for_interpolation[i][j]; 
      //PDF_data[i+1][j+1] = interpolate_neville_aitken_grid(delta_values[i], kappa_values[j], &delta_for_interpolation, &kappa_for_interpolation, &PDF_for_interpolation, constants::order_of_interpolation, constants::order_of_interpolation);
      //PDF_data[i+1][j+1] = interpolate_grid_biquadratic(delta_values[i], kappa_values[j], &delta_for_interpolation, &kappa_for_interpolation, &PDF_for_interpolation);
      PDF_data[i+1][j+1] = interpolate_grid_biquadratic(kappa_1_values[i], kappa_2_values[j], &kappa_1_for_interpolation, &kappa_2_for_interpolation, &PDF_for_interpolation);
    }
  }
  
  cout << "var_kappa_1: " << var_kappa_1 << '\n';
  cout << "var_kappa_2: " << var_kappa_2 << '\n';
  cout << "skew_kappa_1: " << skew_kappa_1 << '\n';
  cout << "skew_kappa_2: " << skew_kappa_2 << '\n';
...
void FlatInhomogeneousUniverseLCDM::compute_LOS_projected_PDF_incl_kappa_saddle_point(double theta, double f_NL, double var_NL_rescale, double kappa_min, double kappa_max, double kappa_noise_variance, vector<double> w_values, vector<double> kernel_values, vector<double> lensing_kernel_values, vector<vector<double> > *delta_grid, vector<vector<double> > *kappa_grid, vector<vector<double> > *PDF_grid){
  ...
  for(int d = 0; d < N_lambda_delta; d++){
    for(int k = 0; k < N_lambda_kappa; k++){
      ...
      if(PDF_on_lambda_grid[d][k] <= 0.0){
        PDF_on_lambda_grid[d][k] = 0.0;
      }
    }
  }
  
  FILE *F1 = fopen("Data/PDF_on_lambda_grid.dat", "w");
  FILE *F2 = fopen("Data/delta_on_lambda_grid.dat", "w");
  FILE *F3 = fopen("Data/kappa_on_lambda_grid.dat", "w");
  FILE *F4 = fopen("Data/Lkappa_on_lambda_grid.dat", "w");
  fclose(F1);
  fclose(F2);
  fclose(F3);
  fclose(F4);
  ...
  fstream out3;
  fstream out4;
  out1.open("Data/PDF_on_lambda_grid.dat");
  out2.open("Data/delta_on_lambda_grid.dat");
  out3.open("Data/kappa_on_lambda_grid.dat");
  out4.open("Data/Lkappa_on_lambda_grid.dat");
  out1 << scientific << setprecision(10);
  out2 << scientific << setprecision(10);
  out3 << scientific << setprecision(10);
  out4 << scientific << setprecision(10);
  
  
  for(int d = 0; d < N_lambda_delta; d++){
    for(int k = 0; k < N_lambda_kappa; k++){
      out1 << PDF_on_lambda_grid[d][k] << setw(20);
      out2 << dphi_dldelta_grid[d][k] << setw(20);
      out3 << dphi_dlkappa_grid[d][k] << setw(20);
      out4 << phi_data_kappa[0][k] << setw(20);
    }
    out1 << '\n';
    out2 << '\n';
    out3 << '\n';
