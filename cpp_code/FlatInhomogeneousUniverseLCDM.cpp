...
FlatInhomogeneousUniverseLCDM::FlatInhomogeneousUniverseLCDM(cosmological_model cosmo, double a_min, double a_max) : FlatHomogeneousUniverseLCDM(cosmo, a_min, a_max){
  ...
  this->Takahashi_shell_thickness = 0;
  if(this->Takahashi_shell_thickness!=0) cout << "TAKAHASHI MODE\n";
  ...
  //this->set_transfer_function_from_file("./Data/joint_PDFs_for_hack/Tk_Takahashi_Class.dat");
...
double FlatInhomogeneousUniverseLCDM::Newtonian_linear_power_spectrum(double k, double e){
  ...
  if(this->power_grid != NULL){
    ...
    return this->power_grid->return_P_L(k, z);
  }
...
vector<vector<double> > FlatInhomogeneousUniverseLCDM::compute_PDF_3D_Francis(double z, double R_in_Mpc_over_h, double f_NL, double b1, double b2, double var_NL_rescale){
  ...
  return this->compute_PDF_from_CGF_Francis_and_Gaussian_bias(&delta_values, &CGF_data, b1, b2);
...
void FlatInhomogeneousUniverseLCDM::compute_tau_coefficients(vector<double>* lambda_1st_branch, vector<double>* phi_1st_branch, vector<double>* phi_prime_1st_branch, vector<double>* phi_prime_prime_1st_branch, vector<double>* coefficients_phi_prime_of_tau, vector<double>* coefficients_dphi_prime_dtau, vector<double>* coefficients_d2phi_prime_dtau2){
  
  int n_lambda = lambda_1st_branch->size();
  vector<double> tau_values(n_lambda, 0.0);
  
  for(int i = 0; i < n_lambda; i++){
    tau_values[i] = sqrt(2.0*((*lambda_1st_branch)[i]*(*phi_prime_1st_branch)[i] - (*phi_1st_branch)[i]));
    if((*lambda_1st_branch)[i] < 0.0) tau_values[i] *= -1.0;
  }
  ...
  double tau_max = 0.9*tau_values[n_lambda-1];
  double tau_min = 0.9*tau_values[0];
  
  int n_coeff = constants::generating_function_coeff_order;
  int n_tau = 4*n_coeff + 1; // has to be odd number in order to include tau=0 exactly.
  ...
  for(int i = 0; i < n_tau; i++){
    phi_prime_for_fit[i] = interpolate_neville_aitken(tau_for_fit[i], &tau_values, phi_prime_1st_branch, constants::order_of_interpolation);
  }
  ...
  (*coefficients_phi_prime_of_tau) = return_coefficients(&tau_for_fit, &phi_prime_for_fit, n_coeff);
  (*coefficients_dphi_prime_dtau) = vector<double>(coefficients_phi_prime_of_tau->size(), 0.0);
  (*coefficients_d2phi_prime_dtau2) = vector<double>(coefficients_phi_prime_of_tau->size(), 0.0);

  /*
   * ISSUE: one can enforce even more coefficients to their analytical value!
   */
  
  double var = interpolate_neville_aitken(0.0, lambda_1st_branch, phi_prime_prime_1st_branch, constants::order_of_interpolation);
...
vector<vector<double> > FlatInhomogeneousUniverseLCDM::compute_PDF_from_CGF(vector<double>* delta_values, vector<vector<double> >* CGF_data){
  ...
  for(int i = 0; i < n_delta; i++){
    ...
    do{
      ...
      //double bL1 = 0.49258917999999996;
      //double bL2 = -0.18765302095238093-0.60912656;
      //b_term_2_next = bL1*b_term_1_next/(1.+phi_prime_next) + 0.5*bL2*b_term_2_next/(1.+phi_prime_next);
      //b_term_2_next = (1.0+log(1.+exp(b_term_2_next)))*(1.+phi_prime_next) - 1.0;
      //b_term_2 = bL1*b_term_1/(1.+phi_prime) + 0.5*bL2*b_term_2/(1.+phi_prime);
      //b_term_2 = (1.0+log(1.+exp(b_term_2)))*(1.+phi_prime) - 1.0;
...
vector<vector<double> > FlatInhomogeneousUniverseLCDM::compute_PDF_from_CGF_Francis(vector<double>* delta_values, vector<vector<double> >* CGF_data){
  ...
  double dr = sqrt(2.0*constants::pi/(n*var));
  // from toolkits paper:
  double lambda_max = n_half*dr;
  double lambda_min = -lambda_max;
  double dl = (lambda_max-lambda_min)/n;
  
  double lambda_values[n];
  double integrand_re[n];
  double integrand_im[n];
  double d_values[n];
  double p_re[n];
  double p_im[n];
  ...
  cout << "CHECKPOINT B\n\n\n";
  double delta_mini = (*delta_values)[0];
  double delta_maxi = (*delta_values)[n_delta-1];
  ...
  for(int i = 0; i < n; i++){
    lambda_values[i] = lambda_min + i*dl;
    lambda = complex<double>(0.0, lambda_values[i]);
    tau = get_tau_from_secant_method_complex_Bernardeau_notation_2D(lambda, tau, &coefficients_dphi_prime_dtau, &coefficients_d2phi_prime_dtau2);
    phi_prime = return_polnomial_value(tau, &coefficients_phi_prime_of_tau);
    exponent = exp(lambda*phi_prime-0.5*pow(tau, 2));
    //
    integrand_re[i] = exponent.real();
    integrand_im[i] = exponent.imag();
    if(i > n_half){
      integrand_re[i] = integrand_re[n-i];
      integrand_im[i] = -integrand_im[n-i];
    }
    //if(phi_prime.real() < 100*delta_mini || phi_prime.real() > 100*delta_maxi){
    //  integrand_re[i] = 0.0;
    //  integrand_im[i] = 0.0;
    //}
    // not needed:
    //if(lambda_values[i]*lambda_values[i] < 0.1*dl*dl){
    //  integrand_re[i] = 1.0;
    //  integrand_im[i] = 0.0;
    //}
  }
  ...
  vector<double> d_array(n);
  vector<double> p_re_array(n);
  
  
  vector<vector<double> > PDF_data(2, vector<double>(n_delta));
  
  for(int i = 0; i < n; i++){
    d_array[i] = d_values[i];
    p_re_array[i] = p_re[i];
    //cout << i <<  "   " << d_array[i] << "   " << p_re_array[i] << "   " << p_im[i] << "     HIHIHI   \n";
  }
  
  for(int i = 0; i < n_delta; i++){
    delta = (*delta_values)[i];
    PDF_data[0][i] = delta;
    PDF_data[1][i] = interpolate_neville_aitken(delta, &d_array, &p_re_array, constants::order_of_interpolation);
    //cout << "\r" << PDF_data[0][i] << "   " << PDF_data[1][i] << "        ";
    //cout << PDF_data[0][i] << "   " << PDF_data[1][i] << "\n";
  }
...
vector<vector<double> > FlatInhomogeneousUniverseLCDM::compute_PDF_from_CGF_Francis_and_Gaussian_bias(vector<double>* delta_values, vector<vector<double> >* CGF_data, double b1, double b2){
  ...
  complex<double> tau(0.0,0.0);
  complex<double> phi_prime;
  complex<double> bias_term_1;
  complex<double> bias_term_2;
  complex<double> delta_L;
  complex<double> delta_L_Sq;
  complex<double> exponent;
  
  vector<vector<double> > PDF_data(3, vector<double>(n_delta));
  ...
  double dr = sigma_frac*min(1.0/sqrt(var), delta_max);
  int log2n = 12;
  int n = pow(2, log2n);
  int n_half = pow(2, log2n-1);
  double lambda_max = n_half*dr;
  ...
  double integrand_re[n];
  double integrand_im[n];
  double integrand_bias_re[n];
  double integrand_bias_im[n];
  double d_values[n];
  double p_re[n];
  double p_im[n];
  double p_bias_re[n];
  ...
  for(int i = 0; i < n; i++){
    ...
    integrand_im[i] = exponent.imag();
    //
    bias_term_1 = return_polnomial_value(tau, &coefficients_bias_term_1_of_tau);
    bias_term_2 = return_polnomial_value(tau, &coefficients_bias_term_2_of_tau);
    delta_L     = bias_term_1/(1.0+phi_prime);
    delta_L_Sq  = bias_term_2/(1.0+phi_prime);
    exponent   *= exp(b1*delta_L + 0.5*b2*delta_L_Sq);
    //
    integrand_bias_re[i] = exponent.real();
    integrand_bias_im[i] = exponent.imag();
  }
  
  cout << "Computing PDF:\n";
  
  fourier_transform(lambda_values, integrand_re, integrand_im, d_values, p_re, p_im, log2n);
  fourier_transform(lambda_values, integrand_bias_re, integrand_bias_im, d_values, p_bias_re, p_bias_im, log2n);
  
  vector<double> d_array(n);
  vector<double> p_re_array(n);
  vector<double> p_bias_re_array(n);
  for(int i = 0; i < n; i++){
    d_array[i] = d_values[i];
    p_re_array[i] = p_re[i];
    p_bias_re_array[i] = p_bias_re[i];
  }
  ...
  for(int i = 0; i < n_delta; i++){
    ...
    PDF_data[0][i] = delta;
    PDF_data[1][i] = interpolate_neville_aitken(delta, &d_array, &p_re_array, constants::order_of_interpolation);
    PDF_data[2][i] = interpolate_neville_aitken(delta, &d_array, &p_bias_re_array, constants::order_of_interpolation);
    //cout << "\r" << PDF_data[0][i] << "   " << PDF_data[1][i] << "   " << PDF_data[2][i] << "        ";
  }
...
vector<vector<double> > FlatInhomogeneousUniverseLCDM::compute_PDF_from_CGF_no_variance_terms(vector<double>* delta_values, vector<vector<double> >* CGF_data){
   ...
   * 
   */
  vector<vector<double> > cut_CGF_data(8, vector<double>(n_lambda, 0.0));
  vector<double> tau_values(n_lambda, 0.0);
  ...
  vector<double> tau_for_fit(n_tau,0.0);
  vector<double> phi_prime_for_fit(n_tau,0.0);
  vector<double> bias_term_0_for_fit(n_tau,0.0);
  vector<double> bias_term_1_for_fit(n_tau,0.0);
  vector<double> bias_term_2_for_fit(n_tau,0.0);
  ...
  for(int i = 0; i < n_tau; i++){
    ...
    bias_term_1_for_fit[i] = interpolate_neville_aitken(tau_for_fit[i], &tau_values, &cut_CGF_data[5], constants::order_of_interpolation);
    bias_term_2_for_fit[i] = interpolate_neville_aitken(tau_for_fit[i], &tau_values, &cut_CGF_data[6], constants::order_of_interpolation);
    bias_term_0_for_fit[i] = interpolate_neville_aitken(tau_for_fit[i], &tau_values, &cut_CGF_data[7], constants::order_of_interpolation);
  }
  ...
  vector<double> coefficients_bias_term_1_of_tau = return_coefficients(&tau_for_fit, &bias_term_1_for_fit, n_coeff);
  vector<double> coefficients_bias_term_2_of_tau = return_coefficients(&tau_for_fit, &bias_term_2_for_fit, n_coeff);
  vector<double> coefficients_bias_term_0_of_tau = return_coefficients(&tau_for_fit, &bias_term_0_for_fit, n_coeff);
  ...
  coefficients_phi_prime_of_tau[0] = 0.0;
  coefficients_phi_prime_of_tau[1] = sqrt(var);
  coefficients_bias_term_0_of_tau[0] = 0.0;
  coefficients_bias_term_1_of_tau[0] = 0.0;
  coefficients_bias_term_2_of_tau[0] = 0.0;
  coefficients_bias_term_2_of_tau[1] = 0.0;
  ...
  complex<double> tau, tau_next;
  complex<double> phi_prime, phi_prime_next;
  complex<double> b_term_0, b_term_0_next;
  complex<double> b_term_1, b_term_1_next;
  complex<double> b_term_2, b_term_2_next;
  complex<double> exponent, exponent_next;
  complex<double> dlambda;
  ...
  vector<vector<double> > PDF_data(5, vector<double>(n_delta));
  ...
  for(int i = 0; i < n_delta; i++){
    ...
    b_term_1 = return_polnomial_value(tau, &coefficients_bias_term_1_of_tau);
    b_term_2 = return_polnomial_value(tau, &coefficients_bias_term_2_of_tau);
    b_term_0 = return_polnomial_value(tau, &coefficients_bias_term_0_of_tau);
    ...
    do{
      ...
      phi_prime_next = return_polnomial_value(tau_next, &coefficients_phi_prime_of_tau);
      exponent_next = exp(-lambda_next*(delta-phi_prime_next)-0.5*pow(tau_next, 2));
      b_term_0_next = return_polnomial_value(tau_next, &coefficients_bias_term_0_of_tau);
      b_term_1_next = return_polnomial_value(tau_next, &coefficients_bias_term_1_of_tau);
      b_term_2_next = return_polnomial_value(tau_next, &coefficients_bias_term_2_of_tau);
      ...
      step = 0.5*dlambda*(exponent_next*b_term_0_next+exponent*b_term_0);
      PDF_data[4][i] += step.imag();
...
vector<vector<double> > FlatInhomogeneousUniverseLCDM::compute_phi_of_lambda_3D(double z, double R, double f_NL, double var_NL_rescale){
  ...
  // delta_L_values == at z=0
  // delta_NL_values == at z=z
  ...
  for(int d = 0; d < n_delta; d++){
     ...
     * <delta_g | delta_m> = delta_m + b_1^L bias_term_1 + 0.5 b_2^L bias_term_2
     * 
     */
...
void FlatInhomogeneousUniverseLCDM::set_current_P_NL(double e){
  this->current_P_NL = this->P_NL(e);
}
