...
extern "C" void return_joint_PDF_kappa_kappa_noisy(double* joint_PDF, double* kappa_values_1, double* kappa_values_2, double theta_in_arcmin, double f_NL, double var_NL_rescale, double kappa_noise_variance_1, double kappa_noise_variance_2, int index_of_source_sample_1, int index_of_source_sample_2){
  
  vector<double> w_values_1;
  vector<double> w_values_2;
  vector<double> dummy;
  vector<double> lensing_kernel_values_1;
  vector<double> lensing_kernel_values_2;
  
  global_universes.projected_galaxy_samples[index_of_source_sample_1]->return_LOS_data(&w_values_1, &dummy, &lensing_kernel_values_1);
  global_universes.projected_galaxy_samples[index_of_source_sample_2]->return_LOS_data(&w_values_2, &dummy, &lensing_kernel_values_2);
  
  vector<vector<double> > rebinned_data = return_joint_binning(w_values_1, lensing_kernel_values_1, w_values_2, lensing_kernel_values_2);
  
  
  
  int N_kappa_1, N_kappa_2;
  
  vector<vector<double> > p_data = global_universes.projected_galaxy_samples[index_of_source_sample_1]->pointer_to_universe()->return_LOS_joint_PDF_kappa_kappa(theta_in_arcmin*constants::arcmin, f_NL, var_NL_rescale, kappa_noise_variance_1, kappa_noise_variance_2, rebinned_data[0], rebinned_data[1], rebinned_data[2]);
    //
  N_kappa_1 = p_data.size()-1;
  N_kappa_2 = p_data[0].size()-1;
  

  
  for(int ik = 0; ik < N_kappa_1; ik++){
    kappa_values_1[ik] = p_data[ik+1][0];
  }
  
  for(int ik = 0; ik < N_kappa_2; ik++){
    kappa_values_2[ik] = p_data[0][ik+1];
  }
  
  int index = 0;
  for(int ik1 = 0; ik1 < N_kappa_1; ik1++){
    for(int ik2 = 0; ik2 < N_kappa_2; ik2++){
      joint_PDF[index] = p_data[ik1+1][ik2+1];
      index++;
    }
  }
...
extern "C" void return_convergence_PDF_direct(double* kappa_values, double* PDF, double* k, double* z, double* P_L_kz_vals, double* P_NL_kz_vals, int n_k, int n_z,const double* z_vals, const double* n_vals_5, int length, double theta_in_arcmin, double Omega_m, double Omega_b, double sigma_8, double n_s, double h_100, double w0, double w1, double A_IA){
  ...
  double a1 = 0.0;
  double a2 = 0.0;
  double density_in_arcmin_squared = 6.0;
  
  double alph_IA = 0.0; // not used

  // // Build vector<vector<double>> with shape (length, 2)
  // std::vector<std::vector<double>> nofz_data;
  // nofz_data.reserve(length);
  // for(int i = 0; i < length; ++i){
  //     nofz_data.emplace_back(std::vector<double>{ z_vals[i], n_vals[i] });
  // }

  // optional: quick monotonicity check on z
  for(int i = 1; i < length; ++i) {
      if(!(z_vals[i] > z_vals[i-1])) {
          std::cerr << "Warning: z array not strictly increasing at index " << i << "\n";
          break;
      }
  }

  int Nk = -1;  // size of kappa grid (determined from first bin)
  ...
  for(int b = 0; b < 5; ++b){
      ...
      // Build n(z) for this bin
      std::vector<std::vector<double>> nofz_data;
      nofz_data.reserve(length);

      const double* n_vals = n_vals_5 + b * length;

      for(int i = 0; i < length; ++i){
          nofz_data.emplace_back(
              std::vector<double>{ z_vals[i], n_vals[i] }
          );
      }

      ProjectedGalaxySample* gals =
          new ProjectedGalaxySample(
              uni,
              density_in_arcmin_squared,
              b1, b2, a0, a1, a2,
              A_IA, alph_IA,
              b_model,
              nofz_data
      ...
      std::vector<std::vector<double>> PDF_data =
          gals->return_kappa_PDF_in_angular_tophat(
              theta_in_arcmin, f_NL, var_NL_rescale
          );
      ...
      // Write into extended output arrays
      for(int d = 0; d < Nk; ++d){
          int idx = b * Nk + d;
          kappa_values[idx] = PDF_data[0][d];
          PDF[idx]   = PDF_data[1][d];
