...
ProjectedGalaxySample::ProjectedGalaxySample(FlatInhomogeneousUniverseLCDM* universe, double density_in_arcmin_squared, double b1, double b2, double a0, double a1, double a2, double a_IA, double alph_IA, BIAS_MODEL b_model, string n_of_z_input_file) : GalaxySample(universe, b1, b2, a0, a1, a2, b_model){
  this->A_IA = a_IA;
  this->alpha_IA = alph_IA;
  this->set_n_of_w_data(n_of_z_input_file);
  this->density = density_in_arcmin_squared/pow(constants::pi/(180.0*60.0), 2);
}

ProjectedGalaxySample::ProjectedGalaxySample(FlatInhomogeneousUniverseLCDM* universe, double density_in_arcmin_squared, double b1, double b2, double a0, double a1, double a2, double a_IA, double alph_IA, double a_lens, BIAS_MODEL b_model, string n_of_z_input_file) : GalaxySample(universe, b1, b2, a0, a1, a2, b_model){
  this->A_lens = a_lens;
  this->A_IA = a_IA;
  this->alpha_IA = alph_IA;
  this->set_n_of_w_data(n_of_z_input_file);
...
ProjectedGalaxySample::ProjectedGalaxySample(FlatInhomogeneousUniverseLCDM* universe, double density_in_arcmin_squared, string n_of_z_input_file, string bias_and_shotnoise) : GalaxySample(universe, bias_and_shotnoise){
  this->set_n_of_w_data(n_of_z_input_file);
  this->density = density_in_arcmin_squared/pow(constants::pi/(180.0*60.0), 2);
}

ProjectedGalaxySample::ProjectedGalaxySample(FlatInhomogeneousUniverseLCDM* universe, double density_in_arcmin_squared, double a_IA, double alph_IA, string n_of_z_input_file, string bias_and_shotnoise) : GalaxySample(universe, bias_and_shotnoise){
  this->A_IA = a_IA;
  this->alpha_IA = alph_IA;
  this->set_n_of_w_data(n_of_z_input_file);
  this->density = density_in_arcmin_squared/pow(constants::pi/(180.0*60.0), 2);
...
ProjectedGalaxySample::ProjectedGalaxySample(FlatInhomogeneousUniverseLCDM* universe, double density_in_arcmin_squared, double a_IA, double alph_IA, double a_lens, string n_of_z_input_file, string bias_and_shotnoise) : GalaxySample(universe, bias_and_shotnoise){
  this->A_lens = a_lens;
  this->A_IA = a_IA;
  this->alpha_IA = alph_IA;
  this->set_n_of_w_data(n_of_z_input_file);
...
ProjectedGalaxySample::ProjectedGalaxySample(FlatInhomogeneousUniverseLCDM* universe, double density_in_arcmin_squared, double b1, double b2, double a0, double a1, double a2, double a_IA, double alph_IA, BIAS_MODEL b_model, const std::vector<std::vector<double>>& nofz_data) : GalaxySample(universe, b1, b2, a0, a1, a2, b_model){
  this->A_IA = a_IA;
  this->alpha_IA = alph_IA;
  this->set_n_of_w_data(nofz_data);
  this->density = density_in_arcmin_squared/pow(constants::pi/(180.0*60.0), 2);
}
...
void ProjectedGalaxySample::set_parameters_projected(
    double density_in_arcmin_squared,
    double b1, double b2,
    double a0, double a1, double a2,
    double A_IA, double alpha_IA,
    BIAS_MODEL b_model,
    const std::vector<std::vector<double>>& nofz_data
){
  this->set_n_of_w_data(nofz_data);
  this->density = density_in_arcmin_squared/pow(constants::pi/(180.0*60.0), 2);
  this->set_parameters(b1, b2, a0, a1, a2, b_model);
 ...
 * 
 */
void ProjectedGalaxySample::set_n_of_w_data(const std::string& n_of_z_input_file){

  auto nofz_data = read_table_double(n_of_z_input_file);
  set_n_of_w_data_core(nofz_data);
}

void ProjectedGalaxySample::set_n_of_w_data(const std::vector<std::vector<double>>& nofz_data){
    set_n_of_w_data_core(nofz_data);
}

void ProjectedGalaxySample::set_n_of_w_data_core(const std::vector<std::vector<double>>& nofz_data){

  if(nofz_data.size() < 2) { /* error */ }
  for(size_t i=1;i<nofz_data.size();++i) {
    if(nofz_data[i].size() < 2) { /* error */ }
  }
  ...
  for(int i = 0; i < Nz-1; i++){
    ...
    Om = this->pointer_to_universe()->return_Omega_m();
    this->lensing_kernel_values[i] *= 1.5*this->A_lens*Om/scale;
    D_growth = this->pointer_to_universe()->return_D_of_eta(eta_0 - w);
    
    // cf. equation A.5 of https://arxiv.org/pdf/2304.01187
    this->lensing_kernel_values[i] -= this->A_IA*pow((1+z)/(1+ this->z0), alpha_IA)*constants::c1_IA_in_units_of_invRho_crit*Om/D_growth*this->n_of_w_values[i];
  }
}

const std::vector<double>&
ProjectedGalaxySample::return_w_values() const {
    return this->w_values;
}

const std::vector<double>&
ProjectedGalaxySample::return_lensing_kernel_values() const {
    return this->lensing_kernel_values;
}
...
void ProjectedGalaxySample::return_joint_saddle_point_PDF_Ng_kappaCMB_noisy_in_angular_tophat(double theta_in_arcmin, double f_NL, double var_NL_rescale, double kappa_min, double kappa_max, double kappa_CMB_noise_variance, vector<vector<double> > *Ng_grid, vector<vector<double> > *kappa_grid, vector<vector<double> > *PDF_grid){
  ...
  if(this->grid_initialised == 0){
    ...
    eta_0 = this->pointer_to_universe()->eta_at_a(1.0);
    w_last_scattering = eta_0-this->pointer_to_universe()->eta_at_a(1.0/(1.0+constants::z_last_scattering));
    ...
    vector<double> kernel_values_extended_to_last_scattering = n_of_w_values;
    ...
    z = 1.0/this->pointer_to_universe()->a_at_eta(eta_0 - w)-1.0;
    ...
    if(this->pointer_to_universe()->return_Takahashi_shell_thickness() == 1){
    ...
    for(int i = 0; i < n_time; i++){
      ...
      z_min = 1.0/this->pointer_to_universe()->a_at_eta(eta_0-w_min)-1.0;
      z_max = 1.0/this->pointer_to_universe()->a_at_eta(eta_0-w_max)-1.0;
      
      a = this->pointer_to_universe()->a_at_eta(eta_0-w);
      z = 1.0/a-1.0;
      CMB_lensing_kernel[i] = 1.5*this->pointer_to_universe()->return_Omega_m()*w*(w_last_scattering-w)/w_last_scattering/a;
    ...
    vector<vector<double> > p_data = this->pointer_to_universe()->return_LOS_joint_PDF_deltaG_kappa(theta_in_arcmin*constants::arcmin, f_NL, var_NL_rescale, kappa_CMB_noise_variance, w_values_extended_to_last_scattering, kernel_values_extended_to_last_scattering, CMB_lensing_kernel);
    this->variance = this->compute_variance_in_angular_tophat(theta_in_arcmin, var_NL_rescale);
    //
    N_delta = p_data.size()-1;
    N_kappa = p_data[0].size()-1;
    this->d_grid = vector<vector<double> >(N_delta, vector<double>(N_kappa, 0.0));
    this->k_grid = vector<vector<double> >(N_delta, vector<double>(N_kappa, 0.0));
    this->p_grid = vector<vector<double> >(N_delta, vector<double>(N_kappa, 0.0));
    double PDF_norm = 0.0;
    double dd = p_data[2][0] - p_data[1][0];
    double dk = p_data[0][2] - p_data[0][1];
    for(int id = 0; id < N_delta; id++){
      for(int ik = 0; ik < N_kappa; ik++){
        this->d_grid[id][ik] = p_data[id+1][0];
        this->k_grid[id][ik] = p_data[0][ik+1];
        this->p_grid[id][ik] = p_data[id+1][ik+1];
        PDF_norm += this->p_grid[id][ik];
      }
    }
    
    PDF_norm *= dd*dk;
    cout << "Norm is now: " << PDF_norm << '\n';
    ...
    N_delta = d_grid.size(); 
    N_kappa = d_grid[0].size(); // in principle N_kappa should be == N_delta. But let's be ignorant on what the other class is doing.
  }
  ...
  vector<vector<double> > P_of_N_given_delta(N_max+1, vector<double>(N_delta, 0.0));
  cout << this->return_bias_model() << " " << N_max << " test 5 \n";
  ...
  for(int k = 0; k < N_kappa; k++){
    for(int n = 0; n < N_max+1; n++){
      ...
      PDF_norm += (*PDF_grid)[n][k];
    }
  }
  PDF_norm *= k_grid[0][2] - k_grid[0][1];
  cout << "Norm is now: " << PDF_norm << '\n';
...
void ProjectedGalaxySample::return_joint_saddle_point_PDF_Ng_kappa_noisy_in_angular_tophat(double theta_in_arcmin, double f_NL, double var_NL_rescale, double kappa_min, double kappa_max, double kappa_noise_variance, vector<double> w_values_lensing_kernel, vector<double> lensing_kernel_values, vector<vector<double> > *Ng_grid, vector<vector<double> > *kappa_grid, vector<vector<double> > *PDF_grid){
  ...
  if(this->grid_initialised == 0){
    ...
    //
    //this->pointer_to_universe()->compute_LOS_projected_PDF_incl_kappa_saddle_point(theta_in_arcmin*constants::arcmin, f_NL, var_NL_rescale, kappa_noise_variance, rebinned_data[0], rebinned_data[1], rebinned_data[2], &this->d_grid, &this->k_grid, &this->p_grid);
    vector<vector<double> > p_data = this->pointer_to_universe()->return_LOS_joint_PDF_deltaG_kappa(theta_in_arcmin*constants::arcmin, f_NL, var_NL_rescale, kappa_noise_variance, rebinned_data[0], rebinned_data[1], rebinned_data[2]);
    this->variance = this->compute_variance_in_angular_tophat(theta_in_arcmin, var_NL_rescale);
    //
    N_delta = p_data.size()-1;
    N_kappa = p_data[0].size()-1;
  ...
  for(int d = 0; d < N_delta; d++){
    norm = 0.0;
    for(int n = 0; n < N_max+1; n++){
      if(this->return_bias_model() == EULERIAN || this->return_bias_model() == TABLE){
        delta_g = this->delta_g_Eulerian(d_grid[d][0], variance);
      }
      else if(this->return_bias_model() == LAGRANGIAN){
        delta_g = interpolate_neville_aitken(d_grid[d][0], &delta_m_and_g[0], &delta_m_and_g[1], constants::order_of_interpolation);
